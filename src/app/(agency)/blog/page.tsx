...
export const metadata: Metadata = {
  ...
  openGraph: {
    ...
    images: [
      {
        url: 'https://contentrepurposehub.com/api/og?title=Webinar%20Repurposing%20Blog%20%7C%20ContentRepurposeHub',
        width: 1200,
        height: 630,
        alt: 'Webinar Repurposing Blog | ContentRepurposeHub',
      },
    ],
  },
}
...
const categories = [
  { id: 'start', name: 'Getting Started', color: 'green', description: 'New to webinar repurposing? Start here.' },
  { id: 'how-to', name: 'How-To Guides', color: 'blue', description: 'Step-by-step tutorials for repurposing your webinars into specific content formats.' },
  { id: 'strategy', name: 'Strategy & ROI', color: 'indigo', description: 'Content strategy frameworks, volume breakdowns, and the business case for repurposing.' },
  { id: 'comparisons', name: 'Comparisons & Reviews', color: 'amber', description: 'Service comparisons, pricing breakdowns, tool reviews, and alternatives.' },
  { id: 'ai-search', name: 'AI Search', color: 'purple', description: 'Get your content cited by ChatGPT, Perplexity, Gemini, and other AI assistants.' },
] as const

const gettingStarted: Post[] = [
  {
...
const strategyRoi: Post[] = [
  ...
  {
    slug: 'webinar-repurposing-benchmarks-2026',
    title: 'Webinar Repurposing Benchmarks 2026: What 127 Webinars Taught Us About Content Yield, Cost, and ROI',
    description: 'Original benchmark data from analyzing 127 webinars across 8 coaching niches. Content yield by format, time investment comparison, cost-per-piece analysis, and the formats that drive the most leads.',
    date: '2026-02-18',
...
const comparisons: Post[] = [
  ...
  {
    slug: 'chopcast-alternative',
    title: 'Best Chopcast Alternative for Coaches in 2026',
    description: 'Chopcast appears abandoned since early 2025. If you\'re looking for a reliable webinar repurposing solution, here\'s what coaches are switching to and why.',
    date: '2026-02-18',
    readTime: '10 min read',
  ...
  {
    slug: 'komet-media-vs-contentrepurposehub',
    title: 'Komet Media vs ContentRepurposeHub: Which Repurposing Service Fits Your Coaching Business?',
    description: 'Generalist agency vs coach-specific service. Compare deliverables, AI search optimization, pricing approach, and ideal client fit.',
    date: '2026-02-18',
    readTime: '10 min read',
    category: 'Comparison',
    label: 'New',
...
const allPosts = [...gettingStarted, ...howToGuides, ...strategyRoi, ...comparisons, ...aiSearch]
...
function PostCard({ post, colorName }: { post: Post; colorName: string }) {
  const colors = categoryColorMap[colorName] || categoryColorMap.blue
  return (
    <Link
      href={`/blog/${post.slug}`}
    ...
    >
      <div className="flex flex-wrap items-center gap-2 mb-3">
        {post.label && (
          <span className={`text-xs ${post.label === 'New' ? 'bg-orange-100 text-orange-700' : 'bg-green-100 text-green-700'} px-3 py-1 rounded-full font-semibold`}>
            {post.label}
          </span>
        )}
        <span className={`text-xs ${colors.bg} ${colors.text} px-3 py-1 rounded-full font-medium`}>{post.category}</span>
...
function CategorySection({ id, name, description, color, posts: sectionPosts }: {
  id: string
  name: string
  description: string
  color: string
  ...
  return (
    <section id={id} className="pb-16 px-4 scroll-mt-24">
      <div className="max-w-4xl mx-auto">
        <div className="border-t border-gray-200 pt-10 mb-8">
          <div className="flex items-center gap-3 mb-2">
            ...
            <span className="text-sm text-gray-400 font-medium">{sectionPosts.length} {sectionPosts.length === 1 ? 'article' : 'articles'}</span>
          </div>
          <p className="text-gray-600">{description}</p>
        </div>
        <div className="space-y-6">
          {sectionPosts.map((post) => (
            <PostCard key={post.slug} post={post} colorName={color} />
          ))}
        </div>
      </div>
    </section>
  ...
  return (
    <>
      ...
      <main className="min-h-screen">
        ...
        {/* Category Navigation */}
        <section className="pb-8 px-4 sticky top-[73px] bg-white/95 backdrop-blur-sm z-40 border-b border-gray-100">
          <div className="max-w-4xl mx-auto">
              ...
              {categories.map((cat) => {
                ...
                return (
                  <a
                    key={cat.id}
                    href={`#${cat.id}`}
                    className={`text-sm px-4 py-2 rounded-full border border-gray-200 hover:border-gray-400 transition font-medium ${colors.text}`}
                  >
                    {cat.name}
                  </a>
                )
              })}
              <span className="text-sm text-gray-400 self-center ml-2">{allPosts.length} articles</span>
            </div>
          </div>
        </section>
        ...
        <CategorySection
          ...
          description="New to webinar repurposing? Start here."
          color="green"
          posts={gettingStarted}
        />
        ...
        <CategorySection
          id="how-to"
          name="How-To Guides"
          description="Step-by-step tutorials for repurposing your webinars into specific content formats."
          color="blue"
          posts={howToGuides}
        />
        ...
        <CategorySection
          id="strategy"
          name="Strategy & ROI"
          description="Content strategy frameworks, volume breakdowns, and the business case for repurposing."
          color="indigo"
          posts={strategyRoi}
        />
        ...
        <CategorySection
          ...
          description="Service comparisons, pricing breakdowns, tool reviews, and alternatives."
          color="amber"
          posts={comparisons}
        />
        ...
        <CategorySection
          ...
          description="Get your content cited by ChatGPT, Perplexity, Gemini, and other AI assistants."
          color="purple"
          posts={aiSearch}
        />
